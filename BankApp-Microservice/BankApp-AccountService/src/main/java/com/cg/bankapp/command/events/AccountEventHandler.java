...
public class AccountEventHandler {
	...
	public void accountUpdateRolledBackEventHandler(AccountUpdateRolledBackEvent event) {
		...
		if(event.getTransactionType().equalsIgnoreCase("Debit")) {
	...
	@EventHandler
	public void  rollBackWithdrawEventHandler(RollBackWithdrawEvent event) throws AccountDetailsNotFoundException {
		com.cg.bankapp.command.entities.Account accountCommandEntity = commandRepository.findById(event.getAccountNo()).orElseThrow(()->new AccountDetailsNotFoundException("Account details not found with id: "+event.getAccountNo()));
		Account accountQueryEntity = queryGateway.query(new GetAccountByIdQuery(event.getAccountNo()), ResponseTypes.instanceOf(Account.class)).join();
		log.info("command db before: "+accountCommandEntity);
		accountCommandEntity.setBalance(event.getBalance());
		commandRepository.save(accountCommandEntity);
		log.info("command db after: "+accountCommandEntity);
		log.info("query db before: "+accountQueryEntity);
		accountQueryEntity.setBalance(event.getBalance());
		queryRepository.save(accountQueryEntity);
		log.info("query db after: "+accountQueryEntity);
	}
}
