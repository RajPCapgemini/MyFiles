...
public class AccountCommandServiceImpl implements AccountCommandService {
	...
	public Account addAccountDetails(Account account) throws InvalidCustomerIdException {
		...
		AccountCreateCommand accountCommand = AccountCreateCommand.builder().uid(UUID.randomUUID().toString())
	...
	@Override
	public Account depositAmount(int accountNo, double amount) throws AccountDetailsNotFoundException,
			HighValueTransactionException, NegativeAmountException, BankServerDownException {
		Account account = getAccountDetails(accountNo);
		if (amount > 100000000)
			throw new HighValueTransactionException("Your account does not have access to make a transaction of amount greater than " + 100000000);
		if (amount < 0)
	...
	@Override
	public List<Account> fundTransfer(int accountNoFrom, int accountNoTo, double amount)
			throws AccountDetailsNotFoundException, HighValueTransactionException, NegativeAmountException,
			InsufficientFundsException, BankServerDownException {
		...
		try {
			...
			return accountList;
		} catch (BankServerDownException e) {
			RollBackWithdrawCommand command = RollBackWithdrawCommand.builder()
					.uid(UUID.randomUUID().toString())
					.accountNo(accountNoFrom)
					.balance(fromAccount.getBalance() + amount)
					.customerId(fromAccount.getCustomerId()).build();
			commandGateway.sendAndWait(command);
			throw new BankServerDownException("Banking Services are currently down. Please try after some time");
		}
	}

	private void addTransaction(TransactionDTO transactionDTO) throws BankServerDownException {
		try {
	...
	private void generateAndExecuteAccountUpdateCommand(Account account) {
		AccountUpdateCommand command = AccountUpdateCommand.builder().uid(UUID.randomUUID().toString())
				.accountNo(account.getAccountNo())
				.balance(account.getBalance())
				.customerId(account.getCustomerId())
				.build();
