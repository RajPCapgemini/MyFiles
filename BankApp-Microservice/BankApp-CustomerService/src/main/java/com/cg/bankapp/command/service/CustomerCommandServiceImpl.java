...
public class CustomerCommandServiceImpl implements CustomerCommandService{
		...
		private void customerAuthentication(UserDTO userDTO) {
			try {
				String uriEndPoint="http://AUTH-SERVER/auth/add";
				String jsonData = new ObjectMapper().registerModule(new JavaTimeModule()).writeValueAsString(userDTO);
				HttpHeaders headers = new HttpHeaders();
				headers.setContentType(MediaType.APPLICATION_JSON);
				
				HttpEntity<String> httpEntity = new HttpEntity<String>(jsonData,headers);
				restTemplate.postForObject(uriEndPoint, httpEntity, UserDTO.class);
			} catch (Exception e) {
